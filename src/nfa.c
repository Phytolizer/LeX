...
static void ConcatenateExpressions(regex_parser_state_t *state, size_t *pStart, size_t *pEnd);
static void ParseExpression(regex_parser_state_t *state, size_t *pStart, size_t *pEnd);
static void ParseTerm(regex_parser_state_t *state, size_t *pStart, size_t *pEnd);
static void ParseFactor(regex_parser_state_t *state, size_t *pStart, size_t *pEnd);
...
static void CleanupTokenMap()
{
    HASH_CLEAR(hh, sTokenMap);
}
...
static char *ExpandMacro(regex_parser_state_t *state)
{
    ...
    *p++ = '\0';
    ...
    if (!macro)
    {
        fprintf(stderr, "in '%s': unknown macro '%s'", state->inputBuf, state->input);
        exit(1);
    }
    state->input = p;
...
static void ConcatenateExpressions(regex_parser_state_t *state, size_t *pStart, size_t *pEnd)
{
    ...
    while (CanBeExpressionStart(state->currentTok))
    {
        ...
        memcpy(state->nodes.data[*pEnd], state->nodes.data[expr2.start], sizeof(nfa_node_t));
...
{
    char first;
    while (state->currentTok != TOK_EOS && state->currentTok != TOK_RIGHT_BRACKET)
    {
